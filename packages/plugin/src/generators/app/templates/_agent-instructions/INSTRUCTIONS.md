...
## Start Here: Read The Widget's Plan File

**Every session that touches a widget starts by reading that widget's plan file:**
`plans/widget-<WidgetName>-blueprint.md`.

It has two parts: `## Current State` — what the widget is right now (purpose, props,
hooks, components, runtime quirks, test/validation status) — and `## Session Log`, an
append-only history of every session that worked on it. Read `## Current State` before
asking the developer questions it already answers, and before assuming anything about
how the widget behaves.

**This applies to quick fixes and continuation sessions too**, not just new widgets.
Those turns skip the process most often, and they are how the file goes stale. Every
widget-touching session appends a session entry, checks off its steps as it completes
them, and updates `## Current State` in the same commit as the code. A session that
edits widget code without touching the plan file is not following these instructions.

If the file does not exist yet, the `build-widget` skill's Step 0 explains how to create
it — via `plan-widget` for new or non-trivial work, or derived from the existing code for
a small fix to an older widget.
